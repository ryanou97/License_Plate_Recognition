...
headers = {'Ocp-Apim-Subscription-Key': key}                            # 查看結果的請求標header
headers_stream = {'Ocp-Apim-Subscription-Key': key,                     # 辨識的請求header
                  'Content-Type': 'application/octet-stream'}



def get_license(img):
    # 將 img 編碼為 JPEG 格式，再將資料轉為 bytes, 此即為要傳送的資料
    # [1]返回資料, [0]返回是否成功
    ...
    while r2.status_code == 200 and r2.json()['status'] != 'Succeeded':
        r2 = requests.get(result_url, headers = headers)    # 持續發出 GET
        time.sleep(0.5)
